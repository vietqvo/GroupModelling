...
def parse_directory(sub_folder):
    ...
    cohesion_writer.writerow(["R", "r", "A", "a", "constant","equil_time", "cd_hat_cd_bar"])
    ...
    for files in os.listdir():
        ...
        if fullfilename[1] == '.csv' and os.path.getsize(files) != 0:       
            ...
            cohesion_writer.writerow([float(df[0]),float(df[1]),float(df[2]),float(df[3]),float(df[4]),float(df[5]),float(df[6])])
