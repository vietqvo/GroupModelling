...
static double total_panic_level;

static Wall * track_areas;
static Py_ssize_t track_area_count;

static double total_interaction_force;
...
{
	return PyFloat_FromDouble(total_panic_level);
}

static PyObject* get_total_interaction_force(PyObject* self)
{
	return PyFloat_FromDouble(total_interaction_force);
}
...
static void calculate_desired_acceleration(Pedestrian * a){
	    ...
	    total_panic_level += a->impatience_level;
...
static void calculate_pedestrian_repulsion(int index)
{
	...
	for(j = 0; j < population_count; j++) {
	        if(index == j) continue;
	        	...
	        	//check  current position of this pedestrian
	        	if( track_area_count > 0) {
	        		if(pedestrians[index].position.x >= track_areas[0].start.x &&
	        		   pedestrians[index].position.x <= track_areas[0].end.x)
	        				total_interaction_force+= vector_length(pedestrians[index].interaction_force_tracking);
	        	}
	}
}
...
{
	PyObject * o, * p_walls, * p_tracking_area;
	...
	p_tracking_area = PyDict_GetItemString(o, "force_tracking_areas");
	track_area_count = PyList_Size(p_tracking_area);
	if(track_area_count >0){
			track_areas    = PyMem_Realloc(track_areas, track_area_count * sizeof(Wall));
			init_walls(p_tracking_area, track_areas, track_area_count);
	}
...
static PyObject * reset_model(PyObject* self)
{
	    ...
	    total_panic_level = 0; //reset the total panic of group
	    total_interaction_force = 0;
	    pedestrians = NULL;
		update_total_count(0);
...
static PyMethodDef ForceModelMethods[] = {
	...
	{"get_total_panic_level",(PyCFunction)get_total_panic_level,METH_NOARGS,
		"Get total panic level"},
	{"get_total_interaction_force",(PyCFunction)get_total_interaction_force,METH_NOARGS,
		"Get total interaction force"},
