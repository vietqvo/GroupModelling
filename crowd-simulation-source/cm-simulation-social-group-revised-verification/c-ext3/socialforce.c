...
static void rk_appropximate_level(int level_k){
	...
	if(level_k==0){ //compute at level 1
		...
		for(i = 0; i < group_population_count; i++) {
			group_pedestrians[i].velocity_rk[0] = group_pedestrians[i].acceleration_rk[0];
			group_pedestrians[i].position_rk[0] = group_pedestrians[i].velocity;
		}
	}
		...
		for(i = 0; i < group_population_count; i++) {
			...
			group_pedestrians[i].position_temp = vector_add(group_pedestrians[i].position, vector_mul(group_pedestrians[i].position_rk[0], timestep));
		}
		...
		for(i = 0; i < group_population_count; i++) {
			group_pedestrians[i].velocity_rk[1] = group_pedestrians[i].acceleration_rk[1];
			group_pedestrians[i].position_rk[1] = vector_add(group_pedestrians[i].velocity,vector_mul(group_pedestrians[i].velocity_rk[0],timestep));
		}
	}
...
static void update_position(Pedestrian * a)
{
	...
	//update position
	delta_p = vector_add(a->position_rk[0],a->position_rk[1]);
	vector_imul(&delta_p,timestep/2);
	a->position = vector_add(a->position,delta_p);


	//update velocity
	delta_v = vector_add(a->velocity_rk[0],a->velocity_rk[1]);
	vector_imul(&delta_v,timestep/2);
...
{
	double radius_sum = a->radius + b.radius;
	Vector from_b     = vector_sub(b.position_temp, a->position_temp);
	double distance   = vector_length(from_b);
...
PyMODINIT_FUNC PyInit_socialforce3(void)
{
	PyObject * m;
