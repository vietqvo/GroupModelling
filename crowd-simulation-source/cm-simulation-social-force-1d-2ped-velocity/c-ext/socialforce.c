...
static void update_position(Pedestrian * a)
{
	...
	temp_position = a->position + delta_p;
	if ((a->initial_position < 0 && temp_position < a->position) || (a->initial_position > 0 && temp_position > a->position)){ //don't allow move backward
		a->position = a->position;
		a->velocity = 0;
		a->is_not_moving = 1;
	} else{
		a->velocity = a->velocity + (a->acceleration * timestep);
		a->position = temp_position;
		a->is_not_moving = 0;
	}
...
static PyObject * a_property(PyObject * self, PyObject * args)
{
	...
	} else if (strcmp(property, "acceleration") == 0) {
		return PyFloat_FromDouble(pedestrians[i].acceleration);
