    ...
    def _generate_elderly_distribution(self,num):
        ...
        if self.parameters['elderly_velocity_deviation'] > 0.0:
            while len(self.elderly_desired_velocities) < num:
                samples = numpy.random.normal(self.parameters['elderly_velocity_mean'], self.parameters['elderly_velocity_deviation'], num*constants.max_samples)
        ...
        if self.parameters['elderly_relaxation_deviation'] > 0.0:    
            while len(self.elderly_relaxation_times) < num:
                samples = numpy.random.normal(self.parameters['elderly_relaxation_mean'], self.parameters['elderly_relaxation_deviation'], num*constants.max_samples)
                self.elderly_relaxation_times.extend(constants._filter_samples_by_mean(samples, num-len(self.elderly_relaxation_times)))
        else:
            self.elderly_relaxation_times = [self.parameters['elderly_relaxation_mean']] * num
            
        if self.parameters['elderly_force_deviation'] > 0.0:
            while len(self.elderly_interaction_strengths) < num:    
                samples =  numpy.random.normal(self.parameters['elderly_force_unit'], self.parameters['elderly_force_deviation'], num*constants.max_samples)
        ...
        if self.parameters['elderly_range_deviation'] > 0.0:
            while len(self.elderly_interaction_ranges) < num:
                samples = numpy.random.normal(self.parameters['elderly_force_range'], self.parameters['elderly_range_deviation'], num)
                self.elderly_interaction_ranges.extend(constants._filter_samples_by_mean(samples, num-len(self.elderly_interaction_ranges)))
        else:
            self.elderly_interaction_ranges = [self.parameters['elderly_force_range']] * num
