...
class Adults(object):
    ...
    def _generate_adults_distribution(self,num):
        ...
        if self.parameters['adult_group_velocity_deviation'] > 0.0:
            while len(self.adults_desired_velocities) < num:
                samples = numpy.random.normal(self.parameters['adult_group_velocity_mean'], self.parameters['adult_group_velocity_deviation'], num)
                self.adults_desired_velocities.extend(constants._filter_samples_by_mean(samples, num-len(self.adults_desired_velocities)))   
        else:
            self.adults_desired_velocities = [self.parameters['adult_group_velocity_mean']] * num
            
        if self.parameters['adult_group_relaxation_deviation'] > 0.0:
            while len(self.adults_relaxation_times) < num:  
                samples = numpy.random.normal(self.parameters['adult_group_relaxation_mean'], self.parameters['adult_group_relaxation_deviation'], num)
                self.adults_relaxation_times.extend(constants._filter_samples_by_mean(samples, num-len(self.adults_relaxation_times)))
        else:
            self.adults_relaxation_times = [self.parameters['adult_group_relaxation_mean']] * num
            
        if self.parameters['adult_group_force_deviation'] > 0.0:
            while len(self.adults_interaction_strengths) < num:        
                samples =  numpy.random.normal(self.parameters['adult_group_force_unit'], self.parameters['adult_group_force_deviation'], num)
            ...
            self.adults_interaction_strengths = [self.parameters['adult_group_force_unit']] * num
            
        if self.parameters['adult_group_range_deviation'] > 0.0:
            while len(self.adults_interaction_ranges) < num:
                samples = numpy.random.normal(self.parameters['adult_group_force_range'], self.parameters['adult_group_range_deviation'], num)
                self.adults_interaction_ranges.extend(constants._filter_samples_by_mean(samples, num-len(self.adults_interaction_ranges)))
        else:
            self.adults_interaction_ranges =  [self.parameters['adult_group_force_range']] * num
