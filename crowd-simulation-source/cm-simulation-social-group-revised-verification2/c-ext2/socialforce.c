...
static void rk_appropximate_level(int level_k){
	...
	if(level_k==0){ //compute at level 1
		for(i = 0; i < group_population_count; i++) {
			...
			group_pedestrians[i].velocity_temp = group_pedestrians[i].velocity;
		...
		for(i = 0; i < group_population_count; i++) {
			...
			for(i = 0; i < group_population_count; i++) {
				...
				group_pedestrians[i].velocity_temp = vector_add(group_pedestrians[i].velocity, vector_mul(group_pedestrians[i].velocity_rk[0], 0.5));
			...
			for(i = 0; i < group_population_count; i++) {
				...
				group_pedestrians[i].velocity_temp = vector_add(group_pedestrians[i].velocity, vector_mul(group_pedestrians[i].velocity_rk[1], 0.5));
			...
			for(i = 0; i < group_population_count; i++) {
				...
				group_pedestrians[i].velocity_temp = vector_add(group_pedestrians[i].velocity, group_pedestrians[i].velocity_rk[2]);
...
static void calculate_desired_acceleration(Pedestrian * a,int level_rk){
	...
	if(a->time_temp) {
		double proj = vector_projection_length(
				a->initial_position, a->target, a->position_temp);
		average_velocity = proj / a->time_temp;
	...
	desired_direction = vector_sub(a->target, a->position_temp);
	vector_unitise(&desired_direction);

	a->acceleration_rk[level_rk]= vector_mul(desired_direction, desired_velocity);

	vector_isub(&a->acceleration_rk[level_rk], &a->velocity_temp);
	vector_imul(&a->acceleration_rk[level_rk], 1.0/a->relax_time);
